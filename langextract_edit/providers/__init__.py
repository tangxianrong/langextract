...
"""Provider package for langextract_edit.
...
from langextract_edit.providers.builtin_registry import BUILTIN_PROVIDERS
from langextract_edit.providers.router import register
from langextract_edit.providers.router import register_lazy
...
def load_plugins_once() -> None:
  ...
  Discovers and loads langextract_edit provider plugins using entry points.
  ...
  if os.environ.get('langextract_edit_DISABLE_PLUGINS', '').lower() in (
      '1',
      'true',
      'yes',
  ):
    logging.info('Plugin loading disabled via langextract_edit_DISABLE_PLUGINS')
  ...
  try:
    ...
    if hasattr(eps, 'select'):
      # Python 3.10+ API
      provider_eps = eps.select(group='langextract_edit.providers')
    elif hasattr(eps, 'get'):
      # Python 3.9 API
      provider_eps = eps.get('langextract_edit.providers', [])
    else:
      # Fallback for older versions
      provider_eps = [
          ep
          for ep in eps
          if getattr(ep, 'group', None) == 'langextract_edit.providers'
      ]
...
def __getattr__(name: str):
  ...
  if name == 'router':
    return importlib.import_module('langextract_edit.providers.router')
  elif name == 'schemas':
    return importlib.import_module('langextract_edit.providers.schemas')
