...
"""Debug utilities for langextract_edit."""
...
_LOG = logging.getLogger("langextract_edit.debug")

# Add NullHandler to prevent "No handler found" warnings
_langextract_edit_logger = logging.getLogger("langextract_edit")
if not _langextract_edit_logger.handlers:
  _langextract_edit_logger.addHandler(logging.NullHandler())
...
def configure_debug_logging() -> None:
  """Enable debug logging for the 'langextract_edit' namespace only."""
  logger = logging.getLogger("langextract_edit")

  # Skip if we already added our handler
  our_handler_exists = any(
      isinstance(h, logging.StreamHandler)
      and getattr(h, "langextract_edit_debug", False)
  ...
  if non_null_handlers:
    ...
    handler.langextract_edit_debug = True
