...
"""Progress and visualization utilities for langextract_edit."""
...
def create_download_progress_bar(
  ...
  return tqdm.tqdm(
      ...
      unit_scale=True,
      desc=(
          f"{BLUE}{BOLD}langextract_edit{RESET}: Downloading"
...
def create_save_progress_bar(
  ...
  return tqdm.tqdm(
      desc=(
          f"{BLUE}{BOLD}langextract_edit{RESET}: Saving to {GREEN}{filename}{RESET}"
      ),
      unit=" docs",
      disable=disable,
  )
...
def create_load_progress_bar(
  ...
  if total_size:
    return tqdm.tqdm(
        total=total_size,
        desc=(
            f"{BLUE}{BOLD}langextract_edit{RESET}: Loading {GREEN}{filename}{RESET}"
        ),
        unit="B",
        unit_scale=True,
        disable=disable,
...
def format_extraction_progress(
  ...
  # Base description
  if model_info:
    desc = f"{BLUE}{BOLD}langextract_edit{RESET}: model={GREEN}{model_info}{RESET}"
  else:
    desc = f"{BLUE}{BOLD}langextract_edit{RESET}: Processing"
...
def create_pass_progress_bar(
  ...
  Returns:
    A configured tqdm progress bar.
  """
  desc = f"{BLUE}{BOLD}langextract_edit{RESET}: Extraction passes"
  return tqdm.tqdm(
      total=total_passes,
      desc=desc,
      bar_format=(
